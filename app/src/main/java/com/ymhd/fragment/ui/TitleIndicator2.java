...
public class TitleIndicator2 extends LinearLayout implements View.OnClickListener,
     ...
     * @param attrs
     */
    public TitleIndicator2(Context context, AttributeSet attrs) {
        super(context, attrs);
        setFocusable(true);
    ...
    /*
     * @杩欎釜鏄牳蹇冨嚱鏁帮紝閫夐」鍗℃槸鐢╟anvas鐢诲嚭鏉ョ殑銆傛墍鏈夌殑invalidate鏂规硶鍧囦細瑙﹀彂onDraw
     * 澶ф剰鏄繖鏍风殑锛氬綋椤甸潰婊氬姩鐨勬椂鍊欙紝浼氭湁涓?涓粴鍔ㄨ窛绂伙紝鐒跺悗onDraw琚Е鍙戝悗锛?
     * 灏变細鍦ㄦ柊浣嶇疆閲嶆柊鐢讳笂婊氬姩鏉★紙鍏跺疄灏辨槸鐢荤嚎锛?
     */
    @Override
    ...
    /**
     * 鑾峰彇鎸囧畾涓嬫爣鐨勯?夐」鍗＄殑鍥炬爣璧勬簮id锛堝鏋滆缃簡鍥炬爣鐨勮瘽锛?
     */
    ...
    private int getNum(int pos) {
        int ret = 0;
        if (mTabs != null && mTabs.size() > pos) {
            ret = mTabs.get(pos).getNum();
        }
        return ret;
    }

    //褰撻〉闈㈡粴鍔ㄧ殑鏃跺?欙紝閲嶆柊缁樺埗婊氬姩鏉?
    public void onScrolled(int h) {
        mCurrentScroll = h;
        invalidate();
    }

    //褰撻〉闈㈠垏鎹㈢殑鏃跺?欙紝閲嶆柊缁樺埗婊氬姩鏉?
    ...
    public void init(int startPos, List<TabInfo> tabs, ViewPager mViewPager) {
        ...
        for (int i = 0; i < mTotal; i++) {
            add(i, getTitle(i), getIcon(i), hasTips(i),getNum(i));
        }
    ...
    public void updateChildTips(int postion, boolean showTips) {
        ...
        final TextView tips = (TextView) child.findViewById(R.id.tab_title_tips);
        if (showTips) {
    ...
    protected void add(int index, String label, int icon, boolean hasTips,int num) {
        View tabIndicator;
//        if (index < 2) {
//            tabIndicator = mInflater.inflate(R.layout.title_flow_indicator2, this, false);
//        } else {
            tabIndicator = mInflater.inflate(R.layout.title_flow_indicator_v3, this, false);
//        }
        final TextView tv = (TextView) tabIndicator.findViewById(R.id.tab_title);
        final TextView tips = (TextView) tabIndicator.findViewById(R.id.tab_title_tips);
        if (mTextColor != null) {
    ...
    public synchronized void setCurrentTab(int index) {
        ...
        ((TextView) newTab.findViewById(R.id.tab_title_tips)).setVisibility(View.GONE);
